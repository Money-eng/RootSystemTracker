...
public class TestGaps {
    ...
    public static void main(String[] args) {
		...
		/*		ArrayList<int[]>correspondencesIndices=getSmartCorrespondences(img,candidatePointTop,
		candidatePointBot,radiusX,radiusY,minRsquared,minVariance,radiusSearch,sigma);
			System.out.println("Smart corr have "+correspondencesIndices.size());
...
//			System.out.println("V="+vX+","+vY+",   "+vNorm);
    ...
    public static ArrayList<int[]> getSmartCorrespondences(ImagePlus img, ArrayList<Point2d> arTop,
        ...
        for (int i = 0; i < N; i++) {
//			blocksBot[i]=VitimageUtils.valuesOfBlockDoubleSlice(imgSlice,arBot.get(i).x-radiusX,arBot.get(i)
//			.y-radiusY/*,Z-1*/,arBot.get(i).x+radiusX,arBot.get(i).y+radiusY/*,Z-1*/);
...
/*			System.out.println(blocksBot[i][0]);
        ...
        for (int i = 0; i < N; i++) {
            ...
            if (corr[i][2] < minVariance || corr[i][3] < minVariance) {
                System.out.print("Removed for variance");
            } else if (corr[i][1] < minRsquared) {
                System.out.print("Removed for correlation");
            } else {
                correspondencesTmp.add(new int[]{i, (int) corr[i][0]});
            }
            System.out.println();
        }
        ...
        for (int i = 0; i < correspondences.size(); i++)
            System.out.println("COCO " + correspondences.get(i)[0] + " -> " + correspondences.get(i)[1]);
