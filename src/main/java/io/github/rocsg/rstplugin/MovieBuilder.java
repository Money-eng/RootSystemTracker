        ...
        for (int n = 0; n < N; n++) {
            ...
            for (int x = 0; x < X; x++) {
                for (int y = 0; y < Y; y++) {
                    ...
                    if (((maskRoot[n][indexSpace] & 0xff) > 140)) {//If root segmentation
                        ...
                        if (modelMode1Activated) {
                            int vls = skel[n][indexSpace] & 0xff;
                            if (vls > 0) {
                                if (vls == 1) {//primary
                        ...
                        if (modelMode2Activated) {
                            int vls = skel[skel.length - 1][indexSpace] & 0xff;
                            if (vls > 0) {
                                if (vls == 1) {//primary
            ...
            for (int p = 0; p < P; p++) {
                ...
                for (int x = (int) (xx0 - R); x <= xx0 + R; x++) {
                    ...
                    for (int y = (int) (yy0 - R); y <= yy0 + R; y++) {
                        ...
                        indexSpace = y * X + x;
                        tabMaskGrid[n][indexSpace] = (byte) (Math.max(valMask,
                                tabMaskGrid[n][indexSpace] & 0xff) & 0xff);
                    }
                }
                //Preparing the arrow
                double lisereSize = isPrimary[p] ? lisere : lisere * 0.8;
...
//						else if(Math.abs(dx)>((targetDY-dy)/anisArrow))tabOut[n][indexSpace]=lisereVal;
                        ...
                        if (n < nStart[p] + deltaN)
                            tabIdent[n][indexSpace] = toByte(alpha * toInt(tabIdent[n][indexSpace]));
                    }
                }
            ...
            if (n == 0) {
                for (int x = 0; x < X; x++) {
                    for (int y = 0; y < Y; y++) {
                        ...
                        if (dgAbsx < 4 && ((nearGridX % 10) == 0)) {
                            tabGrid[indexSpace] = (byte) (gridVal & 0xff);
                            continue;
                        }
                        if (dgAbsy < 4 && ((nearGridY % 10) == 0)) {
                            tabGrid[indexSpace] = (byte) (gridVal & 0xff);
                            continue;
                        }
                        if (dgAbsx < 3 && ((nearGridX % 10) == 0)) {
                            tabGrid[indexSpace] = (byte) (gridVal & 0xff);
                            continue;
                        }
                        if (dgAbsy < 3 && ((nearGridY % 10) == 0)) {
                            tabGrid[indexSpace] = (byte) (gridVal & 0xff);
                            continue;
                        }
                        if (dgAbsx < 2 && ((nearGridX % 10) == 0)) {
                            tabGrid[indexSpace] = (byte) (gridVal & 0xff);
                            continue;
                        }
                        if (dgAbsy < 2 && ((nearGridY % 10) == 0)) {
                            tabGrid[indexSpace] = (byte) (gridVal & 0xff);
                            continue;
                        }
                        if (dgAbsx < 1) {
                ...
                for (int x = 0; x < X; x++) {
                    for (int y = 0; y < Y; y++) {
                        indexSpace = y * X + x;
                        int bef = tabMaskGrid[n - 1][indexSpace] & 0xff;
                        int now = tabMaskGrid[n][indexSpace] & 0xff;
                        if (bef > now) {
                            now = bef;
    ...
    public static double[][][] getAsTimeLapseCoords(RootModel rm) {
        ...
        for (int p = 0; p < P; p++) {
            ...
            for (int n = 0; n < N; n++) System.arraycopy(coordRoots[n], 0, coords[n][p], 0, 5);
        }
        return coords;
    }
    ...
    public static int getIndexAft(double tt) {
        ...
        for (int i = 0; i < hoursExtremities.length - 1; i++)
            if ((tt >= hoursExtremities[i]) && (tt < hoursExtremities[i + 1])) return (i);
        return 100000;
    }
