...
//			(i);yPointsHours[i]=hoursExtremities[(int)Math.round(timeInter.get(i))];}
    ...
    public static void reconnectDisconnectedBranches_v2(ImagePlus img2, SimpleDirectedWeightedGraph<CC,
        ...
        while (!finished) {
            ...
            int nStill = 0;
            for (int i = 0; i < Nstop; i++) {
                if (isTheFirstStep) if ((i % 10) == 0)
                    t2.print("Hungarian algo : building initial score matrix, line  " + i + " / " + Nstop);
                for (int j = 0; j < Nstart; j++) {
                    boolean debug = false;
                    ...
                    if (listStop.get(i) == listStart.get(j)) costMatrix[i][j] = PENALTY_COST;
                        ...
                        if (listStop.get(i).associateSuiv == listStart.get(j))
                            costMatrix[i][j] = -VitimageUtils.EPSILON;
                        else {
                            if (listStop.get(i).associateSuiv != null || listStart.get(j).associatePrev != null)
                                costMatrix[i][j] = 10000;
                            else {
                                nStill++;
                                if (listStop.get(i).changedRecently || listStart.get(j).changedRecently) {
                                    if (listStop.get(i) == ccstoptest && listStart.get(j) == ccstarttest) {
                                        debug = true;
                                    }
            ...
            if (bestW >= thresholdScore) {
                System.out.println("Best exceeds. Break loop");
                finished = true;
                continue;
            }
...
// followers, counted in pixels
    ...
    public static double getCostFirstOrderConnection(ConnectionEdge edge, int nDays, double[] hoursExtremities) {
        ...
        if ((source.trunk) && (!target.trunk)) {
            cost += (1 - VitimageUtils.similarity(source.nPixels / deltaHoursSource,
                    target.nPixels / deltaHoursTarget)) + (nDays - 1) + 0.5 + VitimageUtils.EPSILON;
            if (cost <= -1) {
    ...
    public static void reconnectLateralThatWereThereFromTheStart(SimpleDirectedWeightedGraph<CC, ConnectionEdge> graph, ImagePlus sampleImgForDims, PipelineParamHandler pph, int indexBox) {
        ...
        for (CC ccStart : graph.vertexSet()) {
            if (!ccStart.trunk && ccStart.bestIncomingActivatedCC() == null) {//If start of lateral with no entering
                // arc from a trunk
                ...
                //From there, the root is an actual lateral root, grew for the series, but there is no
                // straightforward attachment, by a physical connexion
                ...
                if (cEnd.euclidianDistanceToCC(ccStart) > (pph.getMeanSpeedLateral() * nTimesSeries / 4.0) && nSuccessors > (nTimesSeries / 4.0)) {
                    //This is a forgotten root of a certain size : a quarter of the mean size in length and number of
                    // successors
                ...
                for (ConnectionEdge edge : graph.outgoingEdgesOf(ccTrunk)) {
                    if (edge.activated) {
                        ...
                        if (!ccStart.trunk) {//Here is a lateral start
                            ...
                            if (ccStart.isLateral) {
                                IJ.log("WARNING Part1 : setting lateral an already lateral : " + ccStart);
                                warns[1]++;
                            }
                            ...
                            ccStart.deltaTimeBefore = 0;//TODO one day when reviewing MADe exclusion. This should be
                            // by bin of delta hours
                            ccStart.deltaTimeFromStart = 0;
                            ccStart.deltaTimeHoursBefore = 0;//TODO one day when reviewing MADe exclusion. This
                            // should be by bin of delta hours
                            ...
                            while (ccTmp.bestOutgoingActivatedCC() != null) {
                                ...
                                if (curEdge.hidden)
                                    lenHid += ccTmp.euclidianDistanceToCC(ccTmp.bestOutgoingActivatedCC());
                                ...
                                if (!ccOld.trunk && ccTmp.trunk) {
                                    IJ.showMessage("WARNING : lateral " + ccOld + " incoming to trunk " + ccTmp);
                                    warns[2]++;
                                }
                                ccStart.pathFromStart.add(ccTmp);
                                if (ccTmp.trunk) {
                                    IJ.log("WARNING 2 : setting lateral a trunk : " + ccTmp);
                                    warns[2]++;
                                }
                                if (ccTmp.isLateral) {
                                    IJ.log("WARNING 2 : setting lateral an already lateral : " + ccTmp);
                                    warns[1]++;
                                }
...
//								if(tmp>nEach.length-1)tmp=nEach.length-1;
        ...
        for (CC ccStart : listStart) {
            ...
            if (ccLast.y < minFinalDepthForAcceptingLatRoot /*&& (ccStart.pathFromStart.get(ccStart.pathFromStart
            .size()-1).y<ccStart.y)*/) {
                ccStart.nonValidLatStart = true;
			...
			for(CC ccs:listStart) {
				...
				if(ccs.euclidianDistanceToCC(ccStart)<=pph.getMinDistanceBetweenLateralInitiation()) {
					...
					if(otherSurf>=curSurf && curSurf<pph.getMinLateralStuckedToOtherLateral()) {
						...
						rejectionCause+=" this goes "+ccStart.goesToTheLeft+" is starting at proximity of a bigger
			...
			*/
            ...
            //The same, but can the end of the root being hidden under another lateral ? We check the global
            // topological context
            if (ccLast.day < (dMax)) {
                ...
                for (ConnectionEdge edge1 : graph.incomingEdgesOf(ccLast)) {
                    CC c1 = edge1.source;//A CC neighbour of ccLast that have its apparition day lower (been there in
                    // first)
                ...
                //Step 3-3 : FOr all possible collected CC to test, compute the cost, based on orientation, and
                // reject if it is waaay too long. If not, add it to list incidencial
                ...
                //Reject the possible incident that could not be incident, and which are smaller than 3*average root
// speed
                if (true) {//TODO : incidence lookup desactivated
    ...
    public static void detectSubtleOutliersBasedOnStatistics(SimpleDirectedWeightedGraph<CC, ConnectionEdge> graph,
        ...
        ArrayList<CC> listStart = new ArrayList<CC>();
        for (CC ccStart : graph.vertexSet())
            if (ccStart.isLatStart && (!ccStart.nonValidLatStart)) listStart.add(ccStart);
        ...
        System.arraycopy(nEach, 0, iter, 0, deltaTimeMax);
        double[][][] data = new double[6][deltaTimeMax + 1][];//collected data for every node : lenbefore,lentotal,
        // speedbef,speedtot,surfthis,surftot
    ...
    public static double weightingOfPossibleHiddenEdge_v2(ImagePlus img, SimpleDirectedWeightedGraph<CC,
        ...
        if (dtCross == 0) {
            ...
            if (ccStop.bestIncomingActivatedCC() != null) {
                ind++;
                dtCross += ccStop.hour - ccStop.bestIncomingActivatedCC().hour;
            }
            if (ccStart.bestOutgoingActivatedCC() != null) {
                ind++;
                dtCross += ccStart.bestOutgoingActivatedCC().hour - ccStart.hour;
            }
        ...
        if (nStop > 1) {
            ...
            if (dtStop == 0) {
                ...
                if (ccPrec.bestIncomingActivatedCC() != null) {
                    ind++;
                    dtStop += ccStop.hour - ccStop.bestIncomingActivatedCC().hour;
                }
        ...
        if (nStart > 1) {
            ...
            if (dtStart <= 0) {
                ...
                if (ccSuiv.bestOutgoingActivatedCC() != null) {
                    ind++;
                    dtStart += ccSuiv.bestOutgoingActivatedCC().hour - ccSuiv.hour;
                }
        ...
        for (int i = 1; i <= maxDay; i++) {
            for (CC cc : graph.vertexSet()) {
                ...
                for (ConnectionEdge edge : graph.incomingEdgesOf(cc)) {
                    if ((graph.getEdgeWeight(edge) < minCostConnected) && (graph.getEdgeSource(edge).stamp == 1) /*&&
                     ((cc.day==(graph.getEdgeSource(edge).day+1))  || (graph.getEdgeSource(edge).trunk))*/) {
                        minCostConnected = graph.getEdgeWeight(edge);
                    }
                    if ((graph.getEdgeWeight(edge) < minCost)/* && ((cc.day==(graph.getEdgeSource(edge).day+1)) ||
                    (graph.getEdgeSource(edge).trunk))*/) {
...
//			System.out.println("DEBUG in MIN SPANNING TREE 4 outgoing edge="+edge);
    ...
    /**
     * Every pixel surrounded with data different from zero will be replaced by the most represented value in its
     * surroundings
     */
...
//		imgDatesTmp.show();
        ...
        if (doImages) {
            int nDays = 1 + Math.round(VitimageUtils.maxOfImage(imgDatesTmp));
    ...
    public static SpanningTree<ConnectionEdge> computeMinimumSpanningTreeGraph(SimpleDirectedWeightedGraph<CC,
            ConnectionEdge> graph) {
    ...
    public static void identifyTrunks(SimpleDirectedWeightedGraph<CC, ConnectionEdge> graph) {
        ...
        for (CC cc : graph.vertexSet())
            if ((cc.day == 1) && graph.containsEdge(root, cc)) {
                cc.trunk = true;
                curCC.add(cc);
            }
        System.out.print("\nTrunk computation");
