...
public class CC implements Serializable {
    ...
    public CC(int day, double hour, int n, Roi r, SimpleDirectedWeightedGraph<CC, ConnectionEdge> graph) {
    ...
    public static CC fuseListOfCCIntoSingleCC(List<CC> list) {
        ...
        for (CC cc : list) {
            if (cc.r.getBounds().x < x) x = cc.r.getBounds().x;
            if (cc.r.getBounds().y < y) y = cc.r.getBounds().y;
            if ((cc.r.getBounds().x + cc.r.getBounds().width) > X) X = cc.r.getBounds().x + cc.r.getBounds().width;
            if ((cc.r.getBounds().y + cc.r.getBounds().height) > Y) Y = cc.r.getBounds().y + cc.r.getBounds().height;
        }
    ...
    public double[] nFacets4connexe_V3(CC cc2) {
        ...
        boolean debug = (this == RegionAdjacencyGraphPipeline.getCC(graph, 21, 5084, 2032)) && (cc2 == RegionAdjacencyGraphPipeline.getCC(graph, 22, 5130, 2080));
    ...
    public SimpleWeightedGraph<Pix, Bord> buildConnectionGraphOfComponent(ImagePlus imgSeg, ImagePlus distToExt,
        ...
        for (int x = 0; x < xM; x++)
            for (int y = 0; y < yM; y++) {
                ...
                if ((x < (xM - 1)) && (y < (yM - 1)) && (connexity == 8)) if (tabPix[x + 1][y + 1] != null)
                    this.pixGraph.addEdge(tabPix[x][y], tabPix[x + 1][y + 1], new Bord(tabPix[x][y],
							tabPix[x + 1][y + 1]));
                if ((x < (xM - 1)) && (y > 0) && (connexity == 8)) if (tabPix[x + 1][y - 1] != null)
                    this.pixGraph.addEdge(tabPix[x][y], tabPix[x + 1][y - 1], new Bord(tabPix[x][y],
							tabPix[x + 1][y - 1]));
	...
	/*
	public void interpolateTimeFromReferencePointsUsingSplineInterpolator(PolynomialSplineFunction psf,
	double[]espaceSource) {
		for(Pix p:pixGraph.vertexSet()) {
			...
			if((p.wayFromPrim+p.distOut)>espaceSource[espaceSource.length-1])p.time=psf.value
			(espaceSource[espaceSource.length-1]);
...
/*
	public void interpolateTimeFromReferencePointsUsingLinearInterpolator(double[]espaceSource,double []timeSource) {
		for(Pix p:pixGraph.vertexSet()) {
			...
			SplineAndPolyLineUtils.linearInterpolation(p.wayFromPrim,espaceSource,timeSource);
    ...
    public int[] getSeedFromFacetConnexion(double[] coords, double[] vectPrev, boolean justDebug) {
        ...
        if (x0 == (int) Math.round(x0)) {
            ...
            if (y0 == (int) Math.round(y0)) {
                ...
                if (this.r.contains((int) Math.round(x0 - 0.5), (int) Math.round(y0 - 0.5))) {
                    System.out.println("v1");
                    x0 -= 0.5;
                    y0 -= 0.5;
                } else if (this.r.contains((int) Math.round(x0 - 0.5), (int) Math.round(y0 + 0.5))) {
